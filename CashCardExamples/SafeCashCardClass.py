...
class SafeCashCard(CashCard):
    ...
    #       {__imit__(). deposit(). check_balance()}
    #   withdraw() 매소드만 다시 정의한다
    def withdraw(self, amount_won):
        ...
        if self.check_balance() >= amount_won:
            # 출금한다
            # 상위 클래스의 withdraw 매소드 호출
            # http://stackoverflow.com/quest ions/805066/
...
# 아래의 내용은 이 파일이 imporet 될 때는 실행되지 않음
if "__main__" == __name__:
    ...
    #   SafeCashCard __int__() 메소드를 호출하려고 하지만
    #   SafeCashCard 클래스 안에는 정의되지 않았기 때문에
    #   상위 클래스인 SimpleCashcard __int__() 메소드가 호출된다
    ...
    #   SafeCashCard deposit() 메소들르 호출하려고 하지만
    #   SafeCashCard 클래스 안에는 정의되지 않았기 때문에
    #   상위 클래스인 SimpleCashCard 의 deposit() 메소드가 호출된다
    mySafeCard.deposit(10000)
    mySistersSafeCard.withdraw(200000)
    ...
    #   SafeCashCard check_balance() 메소드를 호출하려고 하지만
    #   SafeCashCard 클래스 안에는 정의되지 않기 때문에
    #   상위 클래스인 SimpleCashCard 의 check_balance() 메소드가 호출된다
